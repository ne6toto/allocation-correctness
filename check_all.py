# Check all text files in the current folder and all subfolders
# For each file check whether each contains a valid allocation
# Each filename is allocation_v_(k1,k2)_i.txt
...
def main():
    ## Locate all allocation files (all files in current folder and all subfolders)
    mypath = ""
    folders = [f for f in listdir() if not isfile(f)]
    onlyfiles = [f for f in listdir() if isfile(f) and f[-4:]==".txt"]
    print(onlyfiles)
    for folder in folders:
        folderpath = join(mypath, folder)
        for f in listdir(folderpath):
            filehandle = join(folderpath, f)
            if isfile(filehandle) and filehandle[-4:]==".txt":
